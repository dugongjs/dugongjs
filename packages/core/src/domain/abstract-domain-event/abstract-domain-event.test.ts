...
describe("AbstractDomainEvent", () => {
    ...
    describe("fromSchema", () => {
        ...
        it("should allow onCreate to be overridden and access validated payload", async () => {
            const schema = z.object({
                value: z.string().min(3)
            });

            let onCreateCalled = false;
            let payloadAtOnCreate: { value: string } | null = null;

            class EventWithOnCreate extends AbstractDomainEvent.fromSchema(schema) {
                public origin = "TestOrigin";
                public aggregateType = "TestAggregate";
                public type = "EventWithOnCreate";
                public version = 1;

                public onCreate(): void {
                    onCreateCalled = true;
                    payloadAtOnCreate = this.getPayload();
                }
            }

            const event = new EventWithOnCreate(faker.string.uuid(), { value: "valid" });

            // First validate the payload
            await event.validatePayload();
            // Then call onCreate (as the aggregate would)
            event.onCreate?.();

            expect(onCreateCalled).toBe(true);
            expect(payloadAtOnCreate).toEqual({ value: "valid" });
        });

        it("should not reach onCreate when validatePayload fails", async () => {
            const schema = z.object({
                value: z.string().min(3)
            });
            ...
            const event = new EventWithOnCreate(faker.string.uuid(), { value: "ab" }); // too short

            await expect(event.validatePayload()).rejects.toThrow(SchemaValidationError);
            // onCreate should not be called since validation failed
            expect(onCreateCalled).toBe(false);
        });

        it("should support async onCreate", async () => {
            const schema = z.object({
                value: z.string()
            });
            ...
            class EventWithAsyncOnCreate extends AbstractDomainEvent.fromSchema(schema) {
                public origin = "TestOrigin";
                public aggregateType = "TestAggregate";
                public type = "EventWithAsyncOnCreate";
                public version = 1;
            ...
            const event = new EventWithAsyncOnCreate(faker.string.uuid(), { value: "test" });

            await event.validatePayload();
            await event.onCreate?.();
